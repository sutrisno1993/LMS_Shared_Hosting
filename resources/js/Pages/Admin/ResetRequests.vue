<template>
  ...
  <AppLayout
    title="Persetujuan Reset Password Guru"
    subtitle="Daftar pengajuan reset password dari Guru / Wali Kelas"
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-black/20 border-b border-white/8">
                ...
                <th class="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nama Guru</th>
                <th class="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">Nomor WhatsApp</th>
            ...
            <tbody>
              ...
              <tr v-else v-for="(req, idx) in pendingRequests" :key="req.id" class="border-b border-white/4 hover:bg-white/2 transition-colors">
                ...
                <td class="px-4 py-3 font-semibold text-white">{{ req.nama_guru }}</td>
                <td class="px-4 py-3 text-center text-xs text-slate-300">{{ req.no_wa }}</td>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-black/10 border-b border-white/8">
                ...
                <th class="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nama Guru</th>
                <th class="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Nomor WhatsApp</th>
            ...
            <tbody>
              <tr v-for="(req, idx) in processedRequests" :key="req.id" class="border-b border-white/4 text-slate-400">
                ...
                <td class="px-4 py-3 text-xs font-medium text-slate-300">{{ req.nama_guru }}</td>
                <td class="px-4 py-3 text-center text-xs text-slate-400">{{ req.no_wa }}</td>
...
const approve = (id) => {
  if (confirm('Apakah Anda yakin ingin menyetujui reset password untuk guru ini?')) {
    router.post(`/admin/reset-requests/${id}/approve`);
  }
};

const reject = (id) => {
  if (confirm('Apakah Anda yakin ingin menolak pengajuan reset password ini?')) {
    router.post(`/admin/reset-requests/${id}/reject`);
  }
};
...
const navigation = [
  {
    label: 'Akademik',
    items: [
      { href: '/admin/dashboard', icon: '📊', label: 'Dashboard Live' },
      { href: '/admin/kehadiran-guru', icon: '👤', label: 'Kehadiran Guru' },
      { href: '/admin/piket', icon: '🛡️', label: 'Manajemen Piket' },
      { href: '/admin/jurnal', icon: '📓', label: 'Jurnal KBM' },
      { href: '/admin/laporan-kasus-siswa', icon: '⚠️', label: 'Kasus Siswa' },
    ],
  },
  {
    label: 'Pengaturan',
    items: [
      { href: '/admin/timeline', icon: '📅', label: 'Kalender Akademik' },
      { href: '/admin/events', icon: '🎉', label: 'Event & Libur' },
      { href: '/admin/wali-kelas', icon: '🏠', label: 'Wali Kelas' },
      { href: '/admin/siswa', icon: '🎒', label: 'Manajemen Siswa' },
      { href: '/admin/kuesioner', icon: '📝', label: 'Kuesioner Evaluasi' },
      { href: '/admin/grade-config', icon: '⚙️', label: 'Konfigurasi Rapor' },
