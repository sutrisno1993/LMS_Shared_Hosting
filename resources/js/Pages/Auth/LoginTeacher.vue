    ...
    <div class="relative z-10 w-full max-w-md px-4">
      ...
      <div class="glass-card p-10 shadow-2xl shadow-black/50 rounded-3xl" style="background: rgba(255,255,255,0.03); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.05);">
        ...
        <div class="text-center mb-9">
          <div class="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-3xl shadow-xl shadow-indigo-500/40">
            {{ intent === 'walikelas' ? '🏠' : '📚' }}
          </div>
          <h1 class="text-xl font-bold text-white">Login {{ intent === 'walikelas' ? 'Wali Kelas' : 'Guru' }}</h1>
          <p class="text-xs text-slate-400 mt-1">Gunakan No. WhatsApp Anda</p>
        </div>

        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">No WhatsApp</label>
            <div class="relative">
            ...
            <p v-if="form.errors.no_wa" class="text-red-400 text-xs mt-1">{{ form.errors.no_wa }}</p>
          </div>

          <div>
            <label class="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Password / PIN</label>
            <div class="relative">
              <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500">🔒</span>
              <input
          ...
          <button
            ...
            class="w-full relative overflow-hidden py-4 rounded-xl text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed mt-2"
